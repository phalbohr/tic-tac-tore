...
describe('UserPreferencesSection.vue (Story 6.2 & 6.5)', () => {
  ...
  beforeEach(() => {
    pinia = createTestingPinia({
      ...
      initialState: {
        ...
        ruleConfig: {
          presets: [{ id: 'rule-preset-1', name: 'ITSF Standard Matchplay', type: 'PRESET' }],
          customRules: [{ id: 'rule-custom-1', name: 'Fast 7', type: 'CUSTOM' }],
          loading: false,
        },
      },
    })
  ...
  it('[P0] should render selectors for default player group and rule template with options and "None"', async () => {
    ...
    expect(
      groupOptions.some((opt) => opt.text().includes('None') || opt.text().includes('Keine')),
    ).toBe(true)
    ...
    expect(
      ruleOptions.some((opt) => opt.text().includes('None') || opt.text().includes('Kein')),
    ).toBe(true)
