...
describe('MatchScoring', () => {
  ...
  it('enforces position swap for Game 2', async () => {
    ...
    if (selects[0]) await selects[0].setValue('creator-teammate') // Alice Attacker, Bob Defender
    if (selects[1]) await selects[1].setValue('opponent1-opponent2') // Charlie Attacker, David Defender
  ...
  it('allows finishing after 2 games if score is 2-0', async () => {
    ...
    const selects = wrapper.findAll('select')
    if (selects[0]) await selects[0].setValue('creator-teammate')
    if (selects[1]) await selects[1].setValue('opponent1-opponent2')
