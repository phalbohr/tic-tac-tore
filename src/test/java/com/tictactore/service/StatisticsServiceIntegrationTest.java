...
class StatisticsServiceIntegrationTest {
        ...
        void setUp() {
                ...
                playerA = userRepository
                                .save(User.builder().nickname("Alice").email("alice@test.com").avatar("a.png").build());
                playerB = userRepository
                                .save(User.builder().nickname("Bob").email("bob@test.com").avatar("b.png").build());
                playerC = userRepository.save(
                                User.builder().nickname("Charlie").email("charlie@test.com").avatar("c.png").build());
                playerD = userRepository
                                .save(User.builder().nickname("Diana").email("diana@test.com").avatar("d.png").build());
        }
        ...
        void shouldDifferentiatePositionalSynergiesAndCalculateStats() {
                // Match 1: Team A (A=Attacker, B=Defender) vs Team B (C=Attacker, D=Defender)
                // -> Team A wins
                ...
                // Match 2: Team A (A=Attacker, B=Defender) vs Team B (C=Attacker, D=Defender)
                // -> Team A wins
                ...
                // Match 3: Swapped positions! Team A (B=Attacker, A=Defender) vs Team B
                // (C=Attacker, D=Defender) -> Team B wins
                ...
                TeamPairStatsResponse pairAB = result.content().stream()
                                .filter(p -> p.attackerId().equals(playerA.getId())
                                                && p.defenderId().equals(playerB.getId()))
                ...
                TeamPairStatsResponse pairBA = result.content().stream()
                                .filter(p -> p.attackerId().equals(playerB.getId())
                                                && p.defenderId().equals(playerA.getId()))
                                .findFirst().orElseThrow();
                ...
                TeamPairStatsResponse pairCD = result.content().stream()
                                .filter(p -> p.attackerId().equals(playerC.getId())
                                                && p.defenderId().equals(playerD.getId()))
                                .findFirst().orElseThrow();
        ...
        void shouldAggregateRealDatabaseMatchesForHeadToHead() {
                // Match 1 (Vs): A & B vs C & D (A Attacker, B Defender; C Attacker, D Defender)
                // -> A&B wins 10-5
                Match m1 = Match.builder()
                                .creatorId(playerA.getId())
                                .teamAAttackerId(playerA.getId())
                ...
                // Match 2 (With): A & C vs B & D (A Attacker, C Defender; B Attacker, D
                // Defender) -> A&C wins 10-8
                Match m2 = Match.builder()
                                .creatorId(playerA.getId())
                                .teamAAttackerId(playerA.getId())
                ...
                // Goals in Vs (m1): Player A is Attacker, Player C is Attacker => Attacker vs
                // Attacker scored: 10, conceded: 5
