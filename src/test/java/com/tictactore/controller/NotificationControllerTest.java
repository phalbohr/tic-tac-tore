...
class NotificationControllerTest {
    ...
    class SubscriptionEndpoints {
        ...
        void shouldReturnNotificationsForAuthenticatedUser() throws Exception {
            ...
            org.mockito.Mockito
                    .when(pushNotificationService.getUserNotifications(org.mockito.ArgumentMatchers.any(UUID.class)))
                    .thenReturn(List.of(logDto));

            mockMvc.perform(
                    org.springframework.test.web.servlet.request.MockMvcRequestBuilders.get("/api/v1/notifications")
                            .with(csrf()))
                    .andExpect(status().isOk())
                    .andExpect(org.springframework.test.web.servlet.result.MockMvcResultMatchers.jsonPath("$[0].type")
                            .value("MATCH_REJECTED"))
                    .andExpect(org.springframework.test.web.servlet.result.MockMvcResultMatchers.jsonPath("$[0].status")
                            .value("SKIPPED"));
        }
