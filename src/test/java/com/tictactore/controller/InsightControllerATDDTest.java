...
class InsightControllerATDDTest {
    ...
    void setUp() {
        testUser = userRepository.save(User.builder()
                .email("insights-test-" + UUID.randomUUID() + "@example.com")
                .nickname("InsightPlayer" + UUID.randomUUID().toString().substring(0, 5))
                .build());
    }
    ...
    class PlayerInsightsEndpointSpecs {
        ...
        void shouldReturnPlayerInsights_whenAuthenticated() throws Exception {
            mockMvc.perform(get("/api/v1/players/{id}/insights", testUser.getId())
                            .accept(MediaType.APPLICATION_JSON))
                    .andExpect(status().isOk())
                    .andExpect(jsonPath("$.playerId").value(testUser.getId().toString()))
