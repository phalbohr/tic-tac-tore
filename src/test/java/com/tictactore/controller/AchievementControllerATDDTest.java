...
/**
 * ATDD Red-Phase Scaffolds for Achievement REST Controller (GET
 * /api/v1/players/{id}/achievements).
 * Story 7.1: Achievement System (Badges)
 *
 * AC 1: Asynchronous evaluation on match confirmation
 * AC 2: Idempotent awarding of initial 5 achievements (FIRST_WIN, MATCHES_10,
 * CLEAN_SHEET, STRIKER_50, DEFENSE_WALL)
    ...
    void setUp() {
        if (achievementRepository.count() == 0) {
            achievementRepository.saveAll(List.of(
                    Achievement.builder().code("FIRST_WIN").category("MILESTONE")
                            .nameKey("achievements.first_win.title")
                            .descriptionKey("achievements.first_win.description").icon("trophy").build(),
                    Achievement.builder().code("MATCHES_10").category("EXPERIENCE")
                            .nameKey("achievements.matches_10.title")
                            .descriptionKey("achievements.matches_10.description").icon("flame").build(),
                    Achievement.builder().code("CLEAN_SHEET").category("SKILL")
                            .nameKey("achievements.clean_sheet.title")
                            .descriptionKey("achievements.clean_sheet.description").icon("shield").build(),
                    Achievement.builder().code("STRIKER_50").category("OFFENSE")
                            .nameKey("achievements.striker_50.title")
                            .descriptionKey("achievements.striker_50.description").icon("target").build(),
                    Achievement.builder().code("DEFENSE_WALL").category("DEFENSE")
                            .nameKey("achievements.defense_wall.title")
                            .descriptionKey("achievements.defense_wall.description").icon("wall").build()));
        }
    }
