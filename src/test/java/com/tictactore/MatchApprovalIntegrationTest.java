...
@DisplayName("Match Approval API Integration Tests")
@Transactional
public class MatchApprovalIntegrationTest {
    ...
    @Test
    @DisplayName("Successfully approve match when logged in as opponent")
    @WithMockUser(username = "opponent1@test.com")
    void approveMatch_success() throws Exception {
        mockMvc.perform(put("/api/v1/matches/{id}/approve", pendingMatch.getId())
                        .with(csrf()))
                .andExpect(status().isNoContent());

        Match updatedMatch = matchRepository.findById(pendingMatch.getId()).orElseThrow();
        assertThat(updatedMatch.getStatus()).isEqualTo(MatchStatus.CONFIRMED);
    }

    @Test
    @DisplayName("Fail to approve match when logged in as creator")
    @WithMockUser(username = "creator@test.com")
    void approveMatch_failAsCreator() throws Exception {
        mockMvc.perform(put("/api/v1/matches/{id}/approve", pendingMatch.getId())
                        .with(csrf()))
    ...
    @Test
    @DisplayName("Fail to approve match when logged in as teammate")
    @WithMockUser(username = "teammate@test.com")
    void approveMatch_failAsTeammate() throws Exception {
        mockMvc.perform(put("/api/v1/matches/{id}/approve", pendingMatch.getId())
                        .with(csrf()))
                .andExpect(status().isBadRequest());
    ...
    @Test
    @DisplayName("Fail to approve match when match ID not found")
    @WithMockUser(username = "opponent1@test.com")
    void approveMatch_notFound() throws Exception {
        mockMvc.perform(put("/api/v1/matches/{id}/approve", UUID.randomUUID())
                        .with(csrf()))
