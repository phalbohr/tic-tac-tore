...
@Component
public class CleanSheetEvaluator implements AchievementEvaluator {

    public static final String CODE = "CLEAN_SHEET";
    ...
    public boolean evaluate(UUID userId, Match match, PlayerStatsContext stats) {
        ...
        boolean isWinner = (onTeamA && teamAWins > teamBWins) || (onTeamB && teamBWins > teamAWins);
        if (!isWinner) {
            return false;
        }

        if (onTeamA) {
            return match.getGames().stream().allMatch(g -> g.getTeamBScore() == 0);
        } else {
            return match.getGames().stream().allMatch(g -> g.getTeamAScore() == 0);
        }
    }
}
